...
router.post("/checkout", authenticate, (req, res) => {
  ...
  const transaction = db.transaction(() => {
    for (const item of items) {
      const recipes = db.prepare("SELECT * FROM RECIPES WHERE menu_id = ?").all(item.menu_id);
      for (const recipe of recipes) {
        const requiredAmount = recipe.usage_amount * item.qty;
        const ingredient = db.prepare("SELECT current_stock FROM INGREDIENTS WHERE ingredient_id = ?").get(recipe.ingredient_id);
      ...
      if (item.addons && item.addons.length > 0) {
        for (const addon of item.addons) {
          const addonRecipes = db.prepare("SELECT * FROM RECIPES WHERE menu_id = ?").all(addon.menu_id);
...
router.get("/open-bills", authenticate, (req, res) => {
  const bills = db.prepare("SELECT * FROM OPEN_BILLS WHERE status = 'OPEN'").all();
  const billsWithItems = bills.map((bill) => {
    const items = db.prepare("SELECT * FROM OPEN_BILL_ITEMS WHERE bill_id = ?").all(bill.bill_id);
    return { ...bill, items: items.map((i) => ({ ...i, addons: JSON.parse(i.addons || "[]"), is_auto_free: i.is_auto_free === 1 })) };
  });
