...
export default function Dashboard() {
  ...
  const [charts, setCharts] = useState<any>({ salesChart: [], hourlyTraffic: [], paymentMethods: [], topSelling: [] });
  ...
  useEffect(() => {
    fetchDashboard();
  }, [startDate, endDate, paymentMethod]); // Fetch when dates or payment method change
  ...
  const getPeakHour = () => {
    ...
    return charts.hourlyTraffic.reduce((max: any, current: any) => 
        ...
        <div className="h-64">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={charts.hourlyTraffic} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
              ...
              <YAxis yAxisId="right" orientation="right" axisLine={false} tickLine={false} tick={{ fontSize: 12, fill: '#888' }} tickFormatter={(val) => `${val / 1000}k`} />
              <Tooltip cursor={{ fill: '#f9fafb' }} contentStyle={{ borderRadius: '8px', border: 'none', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)' }} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200">
          ...
          <div className="space-y-4">
            {charts.topSelling?.map((item: any, index: number) => (
            ...
            {(!charts.topSelling || charts.topSelling.length === 0) && (
